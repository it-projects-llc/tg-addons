...
function makeShopSteps(expectedAmount) {
    return [
        {
...
function makeEventSteps(expectedAmount) {
    return [
        {
...
function makeStepsCouponAlreadyApplied() {
    return [
        {
...
function makeStepsCouponRefused() {
    return [
        {
            content: "insert discount code",
            extra_trigger: 'form[name="coupon_code"]',
...
registry.category("web_tour.tours").add("panama_not_event_buy", {
    test: true,
    url: "/shop?search=Small%20Cabinet",
    steps: () => makeShopSteps("90.00").concat(makeStepsCouponAlreadyApplied()),
});

registry
    .category("web_tour.tours")
    .add("not_panama_not_event_buy_and_try_apply_" + DISCOUNT_CODE, {
        test: true,
        url: "/shop?search=Small%20Cabinet",
        steps: () => makeShopSteps("100.00").concat(makeStepsCouponRefused()),
    });

registry.category("web_tour.tours").add("panama_event_buy_tickets", {
    test: true,
    url: "/event",
    steps: () => makeEventSteps("900.00").concat(makeStepsCouponAlreadyApplied()),
});
...
registry
    ...
    .add("not_panama_event_buy_tickets_and_try_apply_" + DISCOUNT_CODE, {
        test: true,
        url: "/event",
        steps: () => makeEventSteps("1,000.00").concat(makeStepsCouponRefused()),
    });
